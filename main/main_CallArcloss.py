...
########
    ...
    if config.AUG.MIXUP > 0.:
        ...
        criterion1 = SoftTargetCrossEntropy()
        #添加ARCloss
        criterion = losses.ArcFaceLoss(embedding_size=1784,num_classes=1784).to(torch.device('cuda'))
        loss_optimizer = torch.optim.Adam(criterion.parameters(), lr=0.01)#将0.0001改为0.01

    if config.AMP_OPT_LEVEL != "O0":
        if loss_optimizer:
            model, [optimizer, loss_optimizer] = amp.initialize(model, [optimizer, loss_optimizer], opt_level=config.AMP_OPT_LEVEL)
        else:
...
# ##############
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch_local_data(config, model, criterion, data_loader_train, optimizer, loss_optimizer, epoch, mixup_fn, lr_scheduler)
...
def train_one_epoch_local_data(config, model, criterion, data_loader, optimizer, loss_optimizer, epoch, mixup_fn, lr_scheduler,tb_logger=None):
    model.train()
...
#########    
    ...
    for idx, data in enumerate(data_loader):
        ...
        if config.DATA.ADD_META:
            ...
            outputs = model(samples)  # (feat, x) feat 是倒数第二层输出的特征, x 是最后一层输出的分类概率

        if config.TRAIN.ACCUMULATION_STEPS > 1:
            raise NotImplementedError  # 此分支未按照 loss_optimizer 修改
            optimizer.zero_grad()
            #loss = criterion(outputs, targets)
            _,loss = criterion(outputs, targets)
            ...
            if config.AMP_OPT_LEVEL != "O0":
                if loss_optimizer:
                    with amp.scale_loss(loss, [optimizer, loss_optimizer]) as scaled_loss:
                        scaled_loss.backward()
                else:
                    with amp.scale_loss(loss, optimizer) as scaled_loss:
                        scaled_loss.backward()
        ...
        acc,acc2 = accuracy(outputs, torch.argmax(targets, dim=1), topk=(1,5))
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'tran_acc {acc_meter.avg/100:.4f} \t'
                f'grad_norm {norm_meter.val:.4f} ({norm_meter.avg:.4f})\t'
                f'mem {memory_used:.0f}MB')
    epoch_time = time.time() - start
    logger.info(f"EPOCH {epoch} training takes {datetime.timedelta(seconds=int(epoch_time))}")
    wandb.log({"train_acc":acc_meter.avg/100,
...
@torch.no_grad()
def validate(config, data_loader, model,epoch,mask_meta=False):
#def validate(config, data_loader, model,epoch,mask_meta=False):
    criterion = torch.nn.CrossEntropyLoss()
    model.eval()
...
#################
    ...
    wandb.log({"val_acc1":acc1_meter.avg/100,
               "val_acc5":acc5_meter.avg/100,
...
###
    ...
    # print config
    # logger.info(config.dump())  # TODO 临时屏蔽 
