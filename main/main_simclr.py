...
from pytorch_metric_learning import losses as pml_losses  # huazhen 20230410
import wandb
...
class InfoNCE(torch.nn.Module):
    def __init__(self, batch_size=8, temperature=0.25, is_triplet=True):
        super().__init__()
...
########
    ...
    loss_fn = pml_losses.NTXentLoss(temperature=0.07)  # huazhen 20230410
...
############
        ...
        acc,acc2 = accuracy(outputs, targets, topk=(1,5))
    ...
    for idx, data in enumerate(tqdm(data_loader)):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'Loss {loss_meter.val:.4f} ({loss_meter.avg:.4f})\t'
                f'Acc@1 {acc1_meter.val/100:.3f} ({acc1_meter.avg/100:.3f})\t'
                f'Acc@5 {acc5_meter.val/100:.3f} ({acc5_meter.avg/100:.3f})\t'
                f'Mem {memory_used:.0f}MB')
